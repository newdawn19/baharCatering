package com.bahar.module.schedule;
...
import com.bahar.common.util.RedisLock;
...
public class CouponExpireJob {
    ...
    public void dealCoupon() throws BusinessCheckException, ParseException {
        String lockKey = "lock:couponExpireJob:deal";
        // 唯一标识当前请求/线程
                            ...
                            if (dealNum <= MAX_SEND_NUM) {
                                ...
                                if (couponInfo != null && userInfo != null && StringUtil.isNotEmpty(userInfo.getOpenId())) {
                                    ...
                                    if (days > 1) {
                                        ...
                                        weixinService.sendSubscribeMessage(userInfo.getMerchantId(), userInfo.getId(), userInfo.getOpenId(), WxMessageEnum.COUPON_EXPIRE.getKey(), "pages/user/index", params, new Date());
                                        mtUserCoupon.setUpdateTime(new Date());
                                        mtUserCouponMapper.updateById(mtUserCoupon);
                                    }
                                }
