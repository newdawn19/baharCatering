    ...
    <view class="main-form">
      <view class="footer">
        <view class="btn-wrapper">
          <view class="btn-item btn-item-main" :class="{ disabled }" @click="doSubmit()">确定核销</view>
        </view>
      </view>
    </view>
  </view>
...
<script>
  import * as orderApi from '@/api/merchant/order.js'
  export default {
    data() {
      return {
        code: '',
    ...
    methods: {
      ...
      doSubmit() {
        ...
        app.disabled = true;
        orderApi.confirm({ 'orderId': app.orderId, 'code': app.code })
          .then(result => {
                app.code = '';
                app.disabled = false;
                // 显示提示
                if (parseInt(result.code) === 200) {
                    ...
                    app.$error(result.message ? result.message : "抱歉，核销失败！");
                }
          })
      }
    }
