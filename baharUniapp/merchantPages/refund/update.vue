    ...
    <form @submit="onSubmit()">
      <view class="detail-express b-f m-top20">
        <view class="form-group dis-flex flex-y-center">
          <view class="flex-box">
            <picker mode="selector" :range="statusData" range-key="name" :value="selectedStatusIndex" @change="handlePickerChange">
              <view class="picker">
                状态：{{ statusData[selectedStatusIndex].name }}
               </view>
             </picker>
          </view>
        </view>
        <view class="form-group dis-flex flex-y-center">
          ...
          <view class="flex-box">
            <input class="input" v-model="remark" placeholder="请填写备注信息"></input>
          </view>
        </view>
      </view>
      <!-- 操作按钮 -->
      <view class="footer">
        <view class="btn-wrapper">
          <button class="btn-item btn-item-main btn-normal" :class="{ disabled }" formType="submit">确认更新</button>
        </view>
      </view>
    </form>
...
<script>
  ...
  import * as RefundApi from '@/api/merchant/refund'
  ...
  export default {
    data() {
      return {
        ...
        // 售后单详情
        detail: {},
        statusData: RefundStatusEnum.getData(),
    ...
    methods: {
        ...
        return new Promise((resolve, reject) => {
          RefundApi.detail(app.refundId)
            .then(result => {
              ...
              app.remark = app.detail.remark;
              for (var key = 0; key < app.statusData.length; key++) {
                   const obj = app.statusData[key];
                   if (obj.key == app.detail.status) {
                      app.selectedStatusIndex = key;
                   }
              }
              resolve()
            })
      ...
      onSubmit() {
        ...
        // 按钮禁用
        app.disabled = true;
        const selectedStatusObj = this.statusData[this.selectedStatusIndex];
        app.status = selectedStatusObj.key;
        // 提交到后端
        RefundApi.update({ "refundId": app.refundId, "status": app.status, "remark": app.remark })
          .then(result => {
              if (result.code == 200) {
                  app.$toast("提交成功");
                  setTimeout(() => {
